...
interface LocalTerminalViewerProps {
  tab: PanelTab;
}

export function LocalTerminalViewer({ tab }: LocalTerminalViewerProps) {
  ...
  useEffect(() => {
    ...
    async function init() {
      const [fontSizeStr, fontFamilyStr, shellPathStr, processEnv] =
        await Promise.all([
          getConfig("terminal_font_size").catch(() => null),
          getConfig("terminal_font_family").catch(() => null),
          getConfig("terminal_shell_path").catch(() => null),
          getProcessEnv().catch(() => ({})),
        ]);
      ...
      const shellPath = shellPathStr || "/bin/bash";
      ...
      try {
        ...
        // Set up isolated kubeconfig and switch context, ending with clear
        if (needsSetup) {
          const cmds: string[] = [
            `_K0_SRC="\${KUBECONFIG:-$HOME/.kube/config}"`,
            `export KUBECONFIG=$(mktemp /tmp/k0-kubeconfig-XXXXXX)`,
            `cp "$_K0_SRC" "$KUBECONFIG"`,
            `kubectl config use-context ${context} > /dev/null 2>&1`,
          ];
          if (namespace) {
            cmds.push(`kubectl config set-context --current --namespace=${namespace} > /dev/null 2>&1`);
          }
          cmds.push("clear");
          ptyProcess.write(cmds.join(" && ") + "\n");

          // Reveal after setup + clear have finished (local commands, ~1s is plenty)
          setTimeout(() => {
            if (!cancelled) {
              fitAddon.fit();
              setReady(true);
        ...
        {!ready && (
          <div className="absolute inset-0 flex items-center justify-center z-10 bg-[#1a1a2e]">
            <div className="flex items-center gap-2 text-muted-foreground text-sm">
              ...
              <span>Connecting to {context || "terminal"}...</span>
            </div>
          </div>
        )}
